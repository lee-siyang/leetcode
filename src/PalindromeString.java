...
//        System.out.println(origin.length());
        int coutPalindrome = 0;
        for (int i = 0;i <= origin.length();i++){
            if (isPalindrome(origin,added,i)){
                coutPalindrome++;
            }
        }
//        origin.insert(0,added);
        System.out.println(coutPalindrome);
    }
    private static boolean isPalindrome(String sb, String s, int n){
        StringBuilder stringBuilder = new StringBuilder(sb);
        stringBuilder.insert(n,s);
        String s0 = new String(stringBuilder);
        String s1 = new String(stringBuilder.reverse());
        return s0.equals(s1);
    }
}
